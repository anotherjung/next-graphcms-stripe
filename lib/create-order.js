...
async function createOrder({ sessionId }) {
  ...
  //console.log(314,"create-order",session)

  //console.log(315,"create-order",line_items.data)
  //console.log(316,"create-order",line_items.data[0].price.product.metadata)
    ...
    order: {
      ...
      orderItems: {
        create: line_items.data.map((item) => ({
          ...
          productSizeVariant: {
            connect: {
              id: item.price.product.metadata.variantId
            }
          }
        }))
      }
